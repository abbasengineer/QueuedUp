...
let isSignUpVerified = (data) => {
  ...
  if (isBlank(data.fullName)) errors.fullName = "Must provide full name";
  if (isBlank(data.username)) errors.username = "Must provide username";

  if (isBlank(data.email)) errors.email = "Must provide email";
  else if (!isUCSCEmail(data.email)) errors.email = "Must use @ucsc.edu email";
